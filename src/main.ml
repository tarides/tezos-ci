...
let ci_refs gitlab =
  ...
  let process_refs refs =
    ...
    Gitlab.Api.all_refs refs |> Gitlab.Api.Ref_map.bindings
  ...
  let engine =
    Current.Engine.create ~config:current_config (fun () ->
        Current.all
          ...
          ; Ocaml_ci_gitlab.Pipeline.v ~ocluster:(Option.map snd ocluster)
              ~app:gitlab ~solver ()
          ])
  in
  let site =
...
let cmd =
  ...
  Cmd.v info
    Term.(
      ...
      $ Current_web.cmdliner $ Current_gitlab.Api.cmdliner $ ocluster_cap)
